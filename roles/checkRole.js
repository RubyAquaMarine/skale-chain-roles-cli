...
async function run() {
    ...
    await setDefaultAdminRole(false, GRANT_ROLE_SMART_CONTRACT, CHECK_ADDRESS_ROLE);

    await setRegistrarRole(false, CHECK_ADDRESS_ROLE);

    await setTokenRegisterRole(false, CHECK_ADDRESS_ROLE);

    await setChainConnectorRole(false, CHECK_ADDRESS_ROLE);

    await setDeployerRole(false, CHECK_ADDRESS_ROLE);

    await setDeployerAdminRole(false, CHECK_ADDRESS_ROLE);

    await setMarionette(false, CHECK_ADDRESS_ROLE);

    await setCommunityLocker(false, CHECK_ADDRESS_ROLE);

    await setMessageProxy(false, CHECK_ADDRESS_ROLE);
